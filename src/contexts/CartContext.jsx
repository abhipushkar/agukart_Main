...
const variantsEqual = (a, b) => {
  ...
  for (let i = 0; i < sortedA.length; i++) {
    if (
      sortedA[i].variantName !== sortedB[i].variantName ||
...
const variantDataEqual = (
  ...
  return (
    arraysEqual(variantIdsA, variantIdsB) &&
    arraysEqual(attributeIdsA, attributeIdsB)
  );
};
  ...
  const getCartItems = async (address_id) => {
    ...
    if (token && cartData?.length) {
      ...
      if (res.status === 200) {
        dispatch({
          type: "INIT_CART",
          payload: [...res?.data?.result, ...cartData],
        });
      }
  ...
  useEffect(() => {
    if (!token) {
      ...
      const totalPrice = state?.cart?.reduce((vendorTotal, vendor) => {
        const productTotal = vendor?.products?.reduce(
