...
const MessagePopup = ({
        ...
        products.forEach((product) => {
            productMessages.push({
                ...
                productData: {
                    ...
                    // IMPORTANT FOR FUTURE REVALIDATION
                    qty: product?.qty || 1,
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            if (matchingDocument) {
                if (
                    matchingDocument.data.permanentDeleteUser1 ===
                    senderId
                ) {
                    await updateDoc(
                        doc(db, "chatRooms", matchingDocument.id),
                        {
            ...
            // =========================
                ...
                const productMessages =
                    buildCartProductMessages();

                // append uploaded images in last msg
                if (
                    imageUrls.length > 0 &&
                    productMessages.length > 0
                ) {
                    const lastIndex =
                        productMessages.length - 1;

                    productMessages[lastIndex].imageUrls =
                        imageUrls;
                }
            ...
            if (error.code === "permission-denied") {
                setError(
                    "Firebase Permission Denied. Please update Firebase rules."
                );
