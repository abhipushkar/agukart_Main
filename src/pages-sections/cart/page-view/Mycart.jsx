  ...
  const getAddressData = async () => {
    try {
      ...
      if (res.status == 200) {
        ...
        setDefaultAddress(res?.data?.addresses?.find((item) => item.default == "1"));
      }
    } catch (error) {
            ...
            <Box sx={{ padding: "18px" }}>
              {state?.cart?.length === 0 ? (
                ...
                state?.cart?.map((cart, index) => {
                  return (
                    <SingleVendorCart key={index} wallet={wallet} cart={cart} defaultAddress={defaultAddress} voucherDetails={voucherDetails}/>
                  );
                })
              )}
            </Box>
