...
const Mycart = () => {
  ...
  const { setUserCredentials, usercredentials, getUserDetail } =
    useMyProvider();
  ...
  const { location, setLocation, countries, isLoading, isLoadingCountries } =
    useLocation();
  ...
  useEffect(() => {
    const isCustomizationSelect = state.cart.some((vendor) =>
      vendor.products.some(
        (product) => product.isCombination && product.variantData.length == 0,
      ),
    );
  ...
  useEffect(() => {
    if (token) {
      const shippingAvailable = state.cart.some(
        (vendor) => !vendor.shippingAvailable,
      );
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        const res = await postAPIAuth(
          "user/check-voucher-for-product",
          payload,
        );
        if (res.status === 200) {
  ...
  const handleStoreCouponApply = async () => {
    ...
    try {
      ...
      setStoreCouponLoading(false);
      setStoreCouponError(
        error?.response?.data.message || "Failed to apply store coupon",
      );
      addToast(error?.response?.data.message || error, {
        appearance: "error",
        autoDismiss: true,
  ...
  const handleStoreCouponRemove = async () => {
    ...
    try {
      ...
      const payload = {
        coupon_code:
          storeCouponForm?.store_coupon_code ||
          currentVendor?.vendor_coupon?.coupon_data?.coupon_code,
        vendor_id: currentVendor?.vendor_id,
      };
              ...
              <Grid item lg={12} xs={12}>
                ...
                >
                  ...
                  <Typography component="span" pl={1}>
                    ...
                    with Agukart's Purchase Protection programme for buyers, get
                    a full refund in the rare case your item doesn't arrive,
                                ...
                                <>
                                  ...
                                  <TableCell align="right">
                                    <Typography fontSize={17} component="div">
                                      {currency?.symbol}
                                      {(
                                        +usercredentials?.wallet_balance *
                                        currency?.rate
                                      ).toFixed(2)}
                                    </Typography>
                                  </TableCell>
                        ...
                        <Box mt={2}>
                          ...
                          {isShippingAvailable && (
                            <Typography
                              ...
                              sx={{ mb: 0.5 }}
                            >
                              • Shipping is not available for one or more items
                              in your cart.
                            </Typography>
                          )}
                          {isStockAvailable && (
                            <Typography
                              ...
                              sx={{ mb: 0.5 }}
                            >
                              • Some items are out of stock. Please remove them
                              to proceed.
                            </Typography>
                          )}
                          {isDeleteProduct && (
                            <Typography
                              ...
                              sx={{ mb: 0.5 }}
                            >
                              • Some items are no longer available (deleted).
                              Please remove them.
                            </Typography>
                          )}
                          {isAvailable && (
                        ...
                        <DialogActions>
                          ...
                          <Button
                            onClick={() => {
                              ...
                              const vendorIds = state?.cart?.map(
                                (item) => item?.vendor_id,
                              );
