...
const DrawerVariantSelector = ({
  ...
  // Prevent background scroll when dropdown open (for internal variant)
  useEffect(() => {
    if (open) {
  ...
  useEffect(() => {
    ...
    if (hasAutoScrolledRef.current || !menuRef.current || !selectedAttr) return;
    const el = menuRef.current.querySelector(`[data-attr-id="${selectedAttr.id}"]`);
    if (el) {
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <FormControl fullWidth>
          <Box sx={{ position: "relative", width: "100%" }}>
            <Box ref={triggerRef} onClick={handleToggle} sx={{ m: 0, p: 0, display: "flex", border: "none", background: "#fff", height: "40px", boxShadow: "0 0 3px #000", alignItems: "center", cursor: "pointer", userSelect: "none", borderRadius: "4px" }}>
              <Box sx={{ border: "none", background: "#fff", height: "40px", display: "flex", alignItems: "center", px: 2, borderRadius: "4px" }}>
              ...
              <>
                ...
                <Box
                  ...
                  sx={{
                    ...
                    left: isViewAllOpen ? "50%" : dropdownLeft,
                    transform: isViewAllOpen ? "translate(-50%, -50%)" : "translateY(-50%)",
                ...
                >
                  {!isViewAllOpen && (
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", py: 1, px: 2, cursor: "pointer", "&:hover": { backgroundColor: "#eeeeee" } }}>
                      <Box onClick={() => { onChange(variant.id, ""); setOpen(false); onHoverOut?.(); }} sx={{ flex: 1 }}>Select an option</Box>
                      {variant.viewAllVisible && <Button onClick={handleViewAllClick} size="small" variant="text" sx={{ fontSize: "12px", textTransform: "none", color: "#D23F57", fontWeight: "bold" }}>View All</Button>}
                    </Box>
                  )}
                  ...
                  {isViewAllOpen ? (
                    <Box sx={{ p: 2 }}>
                      ...
                      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "repeat(2, 1fr)", sm: "repeat(auto-fill, minmax(180px, 1fr))" }, gap: 2, overflowY: "auto" }}>
                        {variant.attributes.map((attr) => {
                          ...
                          return (
                            <Box key={attr.id} onClick={() => { if (isDisabled) return; handleImageSelect(attr); }} sx={{ cursor: isDisabled ? "not-allowed" : "pointer", opacity: isDisabled ? 0.5 : 1, position: "relative", border: isSelected ? "2px solid #D23F57" : "1px solid #e0e0e0", borderRadius: "8px", overflow: "hidden", transition: "all 0.2s ease", "&:hover": { transform: isDisabled ? "none" : "translateY(-2px)", boxShadow: isDisabled ? "none" : "0 4px 12px rgba(0,0,0,0.1)" } }}>
                              <Box sx={{ position: "relative", paddingTop: "100%", backgroundColor: "#f5f5f5" }}>
                                {imageSrc ? <img src={imageSrc} alt={attr.value} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover" }} /> : <Box sx={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#f5f5f5", color: "#999" }}>No Image</Box>}
                                {showThumbnail && thumbnailSrc && <Box sx={{ position: "absolute", bottom: "8px", right: "8px", width: "48px", height: "48px", borderRadius: "4px", overflow: "hidden", border: "2px solid white", boxShadow: "0 2px 4px rgba(0,0,0,0.2)", backgroundColor: "white" }}><img src={thumbnailSrc} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></Box>}
                              </Box>
                      ...
                      {variant.attributes.map((attr) => {
                        ...
                        return (
                          <Box
                            ...
                            onClick={() => { if (isDisabled) return; onChange(variant.id, attr.id); setOpen(false); onHoverOut?.(); }}
                            sx={{ backgroundColor: isSelected ? "#ffedf3" : isDisabled ? "#eeeeee" : "inherit", py: 0.5, px: 2, cursor: isDisabled ? "not-allowed" : "pointer", "&:hover": { backgroundColor: isDisabled ? undefined : isSelected ? "#f9e1e1" : "#eeeeee" } }}
                          >
                            <Tooltip title={previewImage ? <Box sx={{ p: 1 }}><img src={previewImage} alt={attr.value} style={{ width: 100, height: 100, objectFit: "cover", borderRadius: 4 }} /></Box> : null} placement="left-start" arrow>
                              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                                <div style={{ display: "flex", alignItems: "center", flex: 1 }}>
                                  {attr.thumbnail && <img src={attr.thumbnail} alt="" style={{ width: "48px", height: "48px", marginRight: "12px", borderRadius: "3px", objectFit: "cover" }} />}
                                  <div style={{ flex: 1, display: "flex", justifyContent: "space-between", gap: "2px" }}>
                                    <span style={{ color: isDisabled ? "#999" : "inherit", fontSize: "14px", fontWeight: 500 }}>{attr.value}</span>
                                    <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                                      {priceText && <span style={{ fontSize: "12px", color: isDisabled ? "#999" : "#666" }}>{priceText}</span>}
                                    </div>
                                  </div>
                                </div>
                                {isDisabled && <Typography variant="caption" sx={{ color: "#d32f2f", fontWeight: "bold", fontSize: "10px", backgroundColor: "#ffebee", padding: "2px 6px", borderRadius: "3px" }}>Sold Out</Typography>}
                              </div>
                            </Tooltip>
                          </Box>
                        );
