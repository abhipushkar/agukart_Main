      ...
      <Box width="100vw" height="100%" position="relative">
        <Scrollbar autoHide={false} sx={{
          ...
          <Box px={5} py={8} maxWidth={500} margin="auto" position="relative" height="100%">
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: "16px", marginLeft: "24px",alignItems:"start" }}>
              <Button onClick={() => navigation("home")} sx={{
                whiteSpace: "nowrap",
              ...
              {cat?.map((cat) => {
                return (
                  <Button onClick={() => navigation(cat)} key={cat._id} sx={{
                    whiteSpace: 'nowrap', borderBottom: "1px solid transparent",
                    "&:hover": {
                      borderBottom: "1px solid black",
                    },
                  }}>
              ...
              {
                usercredentials?.designation_id != "4" && <Button onClick={() => navigation("gift-card-category")} sx={{ whiteSpace: 'nowrap',borderBottom: "1px solid transparent",
                "&:hover": {
                  borderBottom: "1px solid black",
                }, }}>
                  Gift Card
              ...
              {categoryMenus?.map((cat) => {
                return (
                  <Button onClick={() => {
                    router.push(`/products-categories/search/${cat?.slug}?title=${cat?.title}&_id=${cat?._id}`)
                    setOpenDrawer(false);
                  }} key={cat._id} sx={{
                    whiteSpace: 'nowrap', borderBottom: "1px solid transparent",
                    "&:hover": {
